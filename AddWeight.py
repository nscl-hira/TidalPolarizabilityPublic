...
def GetDeformabilityWeight(df):
    exp = ((df['lambda(1.4)'] > 190)*np.exp(-0.5*np.square((df['lambda(1.4)'] - 190)/(390/2.))) 
          + (df['lambda(1.4)'] <= 190)*np.exp(-0.5*np.square((df['lambda(1.4)'] - 190)/(120/2.))))
    return exp
...
def Summarize(args):
  ...
  reasonable = (new_df['lambda(1.4)'] > 0) & (new_df['lambda(1.4)'] < 1200)
